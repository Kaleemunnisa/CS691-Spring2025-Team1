...
sys.path.append('../')
import constants
from utils import(convert_meters_to_pix_distance,
                  convert_pix_to_distance,
                  get_foot_position,
...
class MiniCourt():
    ...
    def convert_meters_to_pixels(self, meters):
        return convert_meters_to_pix_distance(meters, 
    ...
    def set_court_drawing_keypoints(self):
        ...
        #point 4
        drawing_keypoints[8] = drawing_keypoints[0] +  self.convert_meters_to_pixels(constants.double_alley_difference)
        drawing_keypoints[9] = drawing_keypoints[1] 
        # #point 5
        drawing_keypoints[10] = drawing_keypoints[4] + self.convert_meters_to_pixels(constants.double_alley_difference)
        drawing_keypoints[11] = drawing_keypoints[5] 
        # #point 6
        drawing_keypoints[12] = drawing_keypoints[2] - self.convert_meters_to_pixels(constants.double_alley_difference)
        drawing_keypoints[13] = drawing_keypoints[3] 
        # #point 7
        drawing_keypoints[14] = drawing_keypoints[6] - self.convert_meters_to_pixels(constants.double_alley_difference)
    ...
    def draw_background_box(self, frame):
    ...
    def draw_mini_court(self, frames):
        output_frames = []
        for frame in frames:
            frame = self.draw_background_box(frame)
    ...
    def get_mini_court_coords(self, object_position, 
                              closest_keypoint, 
                              closest_keypoint_index, 
                              player_height_in_pixels,
                              player_height_in_meters):
        
        #get distance x and y between the closest keypoint and the player
        ...
        distance_from_keypoint_x_meters = convert_pix_to_distance(distance_from_keypoint_x_pixels, 
                                                                  player_height_in_meters, 
                                                                  player_height_in_pixels)
        distance_from_keypoint_y_meters = convert_pix_to_distance(distance_from_keypoint_y_pixels,
                                                                  player_height_in_meters, 
                                                                  player_height_in_pixels)
        
        #conveert to minicourt coords
        ...
        closest_minicourt_keypoint = (self.drawing_keypoints[closest_keypoint_index*2],
                                      self.drawing_keypoints[closest_keypoint_index*2+1])
        
        minicourt_player_position = (closest_minicourt_keypoint[0] + mini_court_x_distance_in_pixels,
                                     closest_minicourt_keypoint[1] + mini_court_y_distance_in_pixels)
        
        return minicourt_player_position
    

    def convert_bounding_boxes_to_minicourt_coordinates(self, player_boxes, ball_boxes, original_court_keypoints):
        #set players' height
        player_heights = {
        ...
        #go through each frame and player bounding boxes to convert them to mini court coordinates
        for frame_num, player_bbox in enumerate(player_boxes):
            ball_box = ball_boxes[frame_num][1]
            ball_position = get_center_of_bbox(ball_box)
            
            #measure distance between ball position and the center of the center position of the player
            ...
            for player_id, bbox in player_bbox.items():
                ...
                minicourt_player_position = self.get_mini_court_coords(foot_position, 
                                                                      closest_keypoint, 
                                                                      closest_keypoint_index, 
                                                                      max_player_height_in_pixels,
                                                                      player_heights[player_id])
                
                output_player_bbox_dict[player_id] = minicourt_player_position
                ...
                if closest_player_id_to_ball == player_id:
                    ...
                    minicourt_player_position = self.get_mini_court_coords(ball_position, 
                                                                      closest_keypoint, 
                                                                      closest_keypoint_index, 
                                                                      max_player_height_in_pixels,
                                                                      player_heights[player_id])
